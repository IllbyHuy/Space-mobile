...
export default function BookingRequestsScreen() {
  ...
  const fetchRequests = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/PrimaryBookingRequest/GetAll?status=Pending`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
        const safeData = Array.isArray(data) ? data : (data?.data || data?.items || []);
        const myRequests = safeData.filter((req: any) => String(req.lessorId) === String(currentUserId));
  ...
  const updateStatusApi = async (requestId: number, newStatus: 'Approved' | 'Rejected', lesseeId: string) => {
    try {
      const res = await fetch(`${API_BASE}/api/PrimaryBookingRequest/Status/${requestId}`, {
        method: 'PATCH',
        headers: {
      ...
      if (newStatus === 'Approved' && lesseeId) {
        try {
          const convRes = await fetch(`${API_BASE}/api/Conversation/Create?lessorId=${currentUserId}&lesseeId=${lesseeId}`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}` }
          });
    ...
    <View style={styles.container}>
      ...
      {isLoading ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.emptyText}>Không có yêu cầu thuê nào đang chờ duyệt</Text>
        </View>
      ) : (
        <FlatList
          data={requests}
...
const styles = StyleSheet.create({
  ...
  btn: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, paddingHorizontal: 16, borderRadius: 8, gap: 6 },
